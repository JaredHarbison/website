  ...
  module RecruiterAnswerSanitizer
    ...
    def clean(answer)
      CGI.unescapeHTML(answer.to_s)
        ...
        .gsub(/(?:Technology depth varies|His technology depth varies),?\s*(?:so it should be evaluated technology by technology|and should be evaluated by technology rather than assumed to be uniform|rather than assumed to be uniform)/i, "His depth varies by technology, so it is best to look at each one specifically")
